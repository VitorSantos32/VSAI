    ...
    public partial class ModelMenuControl : UserControl
    {
        ...
        private async Task LoadStoreMenuAsync()
        {
            ...
            try
            {
                ...
                string repo = VSAI.Class.Dictionary.dropdownState.ContainsKey("Repo Name") ? VSAI.Class.Dictionary.dropdownState["Repo Name"].ToString() : "VSAI";

                var tasks = new[]
                {
                    FileManager.RetrieveAndAddFiles($"https://api.github.com/repos/{owner}/{repo}/contents/models", "bin\\models", availableModels),
                    FileManager.RetrieveAndAddFiles($"https://api.github.com/repos/{owner}/{repo}/contents/configs", "bin\\configs", availableConfigs)
                };
