    ...
    public partial class StartupWindow : Window
    {
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Verifica e aplica atualizações online
                await AILogic.AutoUpdater.CheckForUpdatesAsync(this);

                // Valida a licença local salva em paralelo com a animação de introdução
                string savedKey = AILogic.LicenseValidator.LoadSavedLicenseKey();
                var licenseTask = Task.Run(() => AILogic.LicenseValidator.ValidateLicenseAsync(savedKey));

                await Task.Delay(1500);

                var (success, _) = await licenseTask;
        ...
        private void UpdateParticleColors(Color baseColor)
        {
            ...
            if (FindName("VSAILogo") is Path logo && logo.Effect is DropShadowEffect shadowEffect)
