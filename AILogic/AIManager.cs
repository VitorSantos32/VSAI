...
namespace VSAI.AILogic
{
    internal class AIManager : IDisposable
    {
        ...
        private bool _isPersistentLock = false;
        ...
        private async Task<bool> InitializeModel(string modelPath)
        {
            using (Benchmark("ModelInitialization"))
            {
                ...
                try
                {
                    if (!await LoadModelAsync(modelPath, selectedEngine))
                    {
                        return false;
                    }

                    _usingEngine = selectedEngine;
                    return true;
                }
                catch (Exception ex)
                {
                    Log(LogLevel.Error, $"Error starting the model via {selectedEngine}: {ex.Message}", true);

                    if (selectedEngine == AIEngine.CUDA || selectedEngine == AIEngine.TensorRT)
                    {
                        MessageBox.Show(
                            $"Falha ao inicializar o motor de processamento GPU ({selectedEngine}).\n\n" +
                            "Isso geralmente ocorre por falta das dependências locais necessárias (como o CUDA Toolkit 12.x e cuDNN da NVIDIA).\n\n" +
                            "O VS AI GLOBAL reconfigurou o motor para 'DirectML' como alternativa para evitar loops de reinicialização.",
                            "Aviso de Dependências GPU - VS AI GLOBAL",
                            MessageBoxButton.OK,
                            MessageBoxImage.Warning);

                        // Revert config to DirectML
                        Dictionary.dropdownState["AI Engine"] = "DirectML";
                        SaveDictionary.WriteJSON(Dictionary.dropdownState, "bin\\dropdown.cfg");

                        // Update the settings dropdown in UI if loaded
                        ...
                        // Try fallback to DirectML
                        try
                        {
                            Log(LogLevel.Info, "Attempting fallback to DirectML...");
                            bool loaded = await LoadModelAsync(modelPath, AIEngine.DirectML);
                            _usingEngine = loaded ? AIEngine.DirectML : _usingEngine;
                            if (loaded) return true;
                        }
                        catch (Exception dmlEx)
                        {
                            Log(LogLevel.Error, $"Error falling back to DirectML: {dmlEx.Message}", true);
                        }
                    }
                    ...
                    try
                    {
                        Log(LogLevel.Info, "Attempting fallback to CPU...");
                        bool loaded = await LoadModelAsync(modelPath, AIEngine.CPU);
                        _usingEngine = loaded ? AIEngine.CPU : _usingEngine;
                        return loaded;
                    }
                    catch (Exception e)
                    {
        ...
        private Task<bool> LoadModelAsync(string modelPath, AIEngine engine)
        {
            try
            {
                OnnxModelLoadResult loadedModel = OnnxModelSessionFactory.Load(modelPath, engine);
        ...
        private bool ValidateOnnxShape()
        {
            if (_onnxModel != null)
            {
                ...
                else
                {
                    ...
                    {
                        Log(LogLevel.Error,
                            $"Output shape does not match the expected shape of {string.Join("x", expectedShape)}.\nThis model will not work with VS AI, please use an YOLOv8 model converted to ONNXv8.",
                            true, 10000);
                        return false;
                    }
        ...
        private void CalculateCoordinates(DetectedPlayerWindow DetectedPlayerOverlay, Prediction closestPrediction, float scaleX, float scaleY)
        {
            ...
            int imageCenterX = IMAGE_SIZE / 2;
            int imageCenterY = IMAGE_SIZE / 2;
            ...
            {
                targetOffsetX = rect.X + (rect.Width * (XOffsetPercentage / 100)) - imageCenterX;
            }
            else
            {
                targetOffsetX = rect.X + rect.Width / 2 - imageCenterX + XOffset;
            }

            if (_isPersistentLock)
            {
                targetOffsetY = 0;
            }
            else if (AimSettings.UseYAxisPercentageAdjustment)
            {
                targetOffsetY = rect.Y + rect.Height - (rect.Height * (YOffsetPercentage / 100)) - imageCenterY + YOffset;
            }
            else
            {
                float yBase = rect.Y;
                ...
                switch (AimSettings.AimingBoundariesAlignment)
                {
                    ...
                    case "Top":
                        // Align to 18% down from the top edge of the bounding box to lock precisely on the head center
                        yAdjustment = rect.Height * 0.18f;
                        break;
                ...
                targetOffsetY = yBase + yAdjustment - imageCenterY + YOffset;
            }

            int halfScreenWidth = (int)DisplayManager.ScreenWidth / 2;
            int halfScreenHeight = (int)DisplayManager.ScreenHeight / 2;

            detectedX = (int)(halfScreenWidth + targetOffsetX);
            detectedY = (int)(halfScreenHeight + targetOffsetY);
        }
        ...
        private void HandleAim(Prediction closestPrediction)
        {
                 ...
                 AimSettings.AimAssist && InputBindingManager.IsHoldingBinding("Second Aim Keybind")))
            {
                ...
                {
                    MouseManager.MoveCrosshair(detectedX, detectedY, _isPersistentLock);
                }
            }
        }
        ...
        private void HandlePredictions(KalmanPrediction kalmanPrediction, Prediction closestPrediction, int detectedX, int detectedY)
        {
            ...
            switch (predictionMethod)
            {
                case "Kalman Filter":
                    ...
                    MouseManager.MoveCrosshair(predictedPosition.X, predictedPosition.Y, _isPersistentLock);
                    break;
                ...
                case "Shall0e's Prediction":
                    ...
                    MouseManager.MoveCrosshair(ShalloePredictionV2.GetSPX(), ShalloePredictionV2.GetSPY(), _isPersistentLock);
                    break;
                ...
                case "wisethef0x's EMA Prediction":
                    ...
                    MouseManager.MoveCrosshair(wtfpredictedPosition.X, wtfpredictedPosition.Y, _isPersistentLock);
                    break;
            }
        }
        ...
        {
            MouseManager.RecordCaptureState();
            ...
            float[] inputArray = _reusableInputArray;

            bool needBitmap = AimSettings.CollectDataWhilePlaying && 
                (!AimSettings.ConstantAiTracking || AimSettings.AutoLabelData) &&
                (DateTime.Now - lastSavedTime).TotalMilliseconds >= SAVE_FRAME_COOLDOWN_MS;

            Bitmap? frame = null;
            bool captureSuccess;
            using (Benchmark("ScreenGrabAndProcess"))
            {
                captureSuccess = _captureManager.CaptureToFloat(
                    detectionBox, 
                    inputArray, 
                    IMAGE_SIZE, 
                    allowStaleCache: _benchmarkMode, 
                    needBitmap: needBitmap, 
                    out frame);
            }

            if (!captureSuccess) return null;
            ...
            try
            {
                ...
                Prediction? finalTarget = _stickyAimSelector.SelectTarget(
                    ...
                    KDPredictions,
                    out isPersistent);
                _isPersistentLock = isPersistent;
            ...
            PerformanceGoal goal = PerformanceGoal.Balanced)
        {
            ...
            AIEngine originalEngine = _usingEngine;
        ...
        private void ReloadBenchmarkModelSession(InferenceSession? preservedSession)
        {
            ...
            OnnxModelLoadResult loadedModel = OnnxModelSessionFactory.Load(_modelPath, _usingEngine);
            _onnxModel = loadedModel.Session;
            _outputNames = loadedModel.OutputNames;
        ...
        private void StopAiLoop()
        {
            ...
            try
            {
                ...
                if (loopTask != null && !loopTask.Wait(TimeSpan.FromSeconds(2)))
                {
                    Log(LogLevel.Warning, "AI loop did not stop within 2 seconds. Proceeding with disposal.");
                }
            }
